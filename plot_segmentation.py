...
print(beats)
# print(librosa.sample_to_time(beats))
print(tempo)
...
# [print("{}{}".format(b//60, b%60)) for b in beat_times]
...
#                          bins_per_octave=BINS_PER_OCTAVE,
#                          x_axis='time', ax=ax)
# for [i0,i1], label in zip(zip(bound_times, bound_times[1:]), bound_segs):
#     # print("{}:{:.2f}\t{}:{:.3f}\t{}".format(int(i0//60),i0%60,int(i1//60),i1%60, label))
#     # print("{:.2f}\t{:.2f}\t{}".format(i0,i1, label))
#     ax.add_patch(patches.Rectangle((i0, freqs[0]),
#                                    i1 - i0,
#                                    freqs[-1],
...
for [i0,i1], label in zip(zip(bound_times, bound_times[1:]), bound_segs):
    ys= librosa.time_to_samples([i0,i1]) 
    y_crop=y[ys[0]:ys[1]]
    tempo1, beats1 = librosa.beat.beat_track(y=y_crop, sr=sr, trim=True)
    print("{:.2f}\t{:.2f}\t{} tempo: {:.3f}".format(i0,i1, label,tempo1))
    # for i,b in enumerate(librosa.frames_to_time(beats1)):
        # print("{:.2f}\t{:.2f}\t beat: {}".format(i0+b,i0+b,i))
